...
for (const source of sources) {
  ...
  for (const row of rows) {
    ...
    if ((source.requiresLiveId && !row.liveId) || !expected || row[source.dateField] !== expected || Object.hasOwn(row, 'replayCtime')) { failed += 1; console.error(`FAIL ${source.file} ${row.id || row.filename || checked}: ${row.broadcastTime}/${row[source.dateField]}, expected ${expected}`) }
  }
